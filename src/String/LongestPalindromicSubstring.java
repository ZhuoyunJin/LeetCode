...
public class LongestPalindromicSubstring {
		...
		for(int i=0;i<len;i++) {
			for(int j=0;j+i<len;j++) {
				if(i==0) {
					...
					flag[j][j+i] = s.charAt(j)==s.charAt(j+1);
				}else {
					flag[j][j+i] = (s.charAt(j)==s.charAt(j+i)) && flag[j+1][j+i-1];
				}

				if(i>maxLength && flag[j][j+i]) {
					startIndex = j;
					maxLength = i;
				}
			}
		}
		//Utility.printArray(flag);
	...
	public static void main(String[] args) {
		...
		System.out.println(l.longestPalindrome4("bb"));
	}
}

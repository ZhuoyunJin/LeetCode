...
public class CloneGraph {
	...
	public static UndirectedGraphNode cloneGraph(UndirectedGraphNode node) {
		...
		while (!queue.isEmpty()) {
			oldNode = queue.poll();
			newNode = map.containsKey(oldNode.label)?map.get(oldNode.label):new UndirectedGraphNode(oldNode.label);
			map.put(newNode.label, newNode);
			for(UndirectedGraphNode n:oldNode.neighbors){
				if(map.containsKey(n.label)){
					newNode.neighbors.add(map.get(n.label));
				}else{
					UndirectedGraphNode node2 = new UndirectedGraphNode(n.label);
